...
export function TinyEditor({
  ...
  // 合并插件配置
  const mergedPlugins = plugins ? [...defaultPlugins, ...plugins] : defaultPlugins;
  
  // 合并工具栏配置
  const mergedToolbar = toolbar ? [...defaultToolbar, ...toolbar] : defaultToolbar;

  // 默认字体设置
  const defaultFontFamilyFormats = 'Arial=arial,helvetica,sans-serif; 宋体=simsun; 微软雅黑=microsoft yahei,sans-serif; 黑体=simhei; 楷体=kaiti; 仿宋=fangsong; 等线=dengxian';
  const defaultFontSizeFormats = '12px 14px 16px 18px 20px 24px 28px 32px 36px 48px 56px 72px';
  const defaultLineHeightFormats = '1 1.2 1.4 1.6 2';
  const defaultBlockFormats = '段落=p; 标题 1=h1; 标题 2=h2; 标题 3=h3; 标题 4=h4; 标题 5=h5; 标题 6=h6; 引用=blockquote; 预格式化=pre; 地址=address; 代码=code';
      ...
      <Editor
        ...
        init={{
          ...
          menu: {
            ...
            format: { title: '格式', items: 'bold italic underline strikethrough superscript subscript codeformat | formats blockformats fontformats fontsizes align | forecolor backcolor | removeformat' },
